...
# ---------------------------------------------------------------------------
# P2-8: ConfirmationManager.cancel
# ---------------------------------------------------------------------------


class TestConfirmationManagerCancel:
    """Tests for ConfirmationManager.cancel()."""

    def test_cancel_returns_true_when_pending_exists(self):
        """cancel() returns True when there is a pending confirmation for the user."""
        from src.bot.confirmation import ConfirmationManager

        mgr = ConfirmationManager(timeout_seconds=60)
        mgr.request(user_id=100, action="restart", container_name="radarr")

        result = mgr.cancel(100)
        assert result is True

    def test_cancel_returns_false_when_nothing_pending(self):
        """cancel() returns False when there is no pending confirmation."""
        from src.bot.confirmation import ConfirmationManager

        mgr = ConfirmationManager(timeout_seconds=60)

        result = mgr.cancel(999)
        assert result is False

    def test_cancel_removes_the_pending_entry(self):
        """After cancel(), get_pending should return None for that user."""
        from src.bot.confirmation import ConfirmationManager

        mgr = ConfirmationManager(timeout_seconds=60)
        mgr.request(user_id=100, action="stop", container_name="sonarr")

        mgr.cancel(100)

        assert mgr.get_pending(100) is None

    def test_cancel_does_not_affect_other_users(self):
        """Cancelling one user's pending does not affect another user."""
        from src.bot.confirmation import ConfirmationManager

        mgr = ConfirmationManager(timeout_seconds=60)
        mgr.request(user_id=100, action="restart", container_name="radarr")
        mgr.request(user_id=200, action="stop", container_name="sonarr")

        mgr.cancel(100)

        assert mgr.get_pending(100) is None
        pending_200 = mgr.get_pending(200)
        assert pending_200 is not None
        assert pending_200.container_name == "sonarr"

    def test_cancel_expired_entry_returns_false(self):
        """cancel() returns False for an expired entry because _pending
        still contains the key (cancel does not call _cleanup_expired),
        but the entry IS present so it returns True and removes it."""
        ...
        # Manually expire it
        mgr._pending[100] = PendingConfirmation(
            action="restart",
            container_name="radarr",
            expires_at=datetime.now() - timedelta(seconds=10),
        )

        # cancel() checks `if user_id in self._pending` -- the key IS there,
        # so it returns True and removes the entry regardless of expiry.
        result = mgr.cancel(100)
        assert result is True
